...
<head>
		...
		<meta charset="utf-8" />
		<title>Manage TimeTable | Update TimeTable</title>
							...
							<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
								...
								<div class="d-flex align-items-center flex-wrap mr-1">
									...
									<div class="d-flex align-items-baseline flex-wrap mr-5">
										<!--begin::Page Title-->
										<h5 class="text-dark font-weight-bold my-1 mr-5">Manage TimeTable</h5>
										<!--end::Page Title-->
										<!--begin::Breadcrumb-->
										<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
											...
											<!-- <li class="breadcrumb-item">
												<a href="#" class="text-muted">Manage Add TimeTable</a>
											</li> -->
											<li class="breadcrumb-item">
												<a href="#" class="text-muted">Update TimeTable</a>
											</li>
									...
									<div class="col-6">
										...
										<div class="card card-custom example example-compact">
											<div class="card-header">
												<h3 class="card-title">update timetable TimeTable</h3>
											...
											{
												$bus_id = $_POST["txtbusid"];
												$bus_station_id = $_POST["txtbusstationid"];
												$timing = $_POST ["txttiming"];
												 
												$result = mysqli_query($conn,"update 
													tbl_timetable set Bus_id='".$bus_id."',Bus_station_id='".$bus_station_id."',Timing='".$timing."'  where Time_id = '".$id."'") or die(mysqli_error($conn));
												if($result==true)
												{
											echo "<script>window.location='timetable.php'</script>";
												}
												else
												{
													echo "Error!";
											...
											<form class="form" method="post" enctype="multipart/form-data">
												<?php
												$count=1;
												$result = mysqli_query($conn,"select * from tbl_timetable where Time_id='".$id."'") or die(mysqli_error($conn));
												...
												<div class="card-body">
													<div class="form-group">
														<label>bus id</label>
														<select name="txtbusid" class="form-control">
														<!-- <option>Please Select Bus Id</option> -->
															<?php
												$result1 = mysqli_query($conn,"select * from tbl_bus") or die(mysqli_error($conn));
												while($row1=mysqli_fetch_assoc($result1))
												{
												?>
											<option value="<?php echo $row1["Bus_id"]; ?>"><?php echo $row1["Bus_no"]; ?></option>
											<?php } ?>

														</select>
													</div>
														<div class="form-group">
														<label>bus station id</label>
														<select name="txtbusstationid" class="form-control">
															<!-- <option>Please Select The Bus Station Id</option> -->
															<?php
												$result2 = mysqli_query($conn,"select * from tbl_bus_station") or die(mysqli_error($conn));
												while($row2=mysqli_fetch_assoc($result2))
												{
												?>
											<option value="<?php echo $row2["Bus_station_id"]; ?>"><?php echo $row2["Bus_station_name"]; ?></option>
											<?php } ?>
														</select>
													</div>
													...
													<div class="form-group">
														<label>Timing</label>
														<input type="text" name="txttiming" class="form-control"value="<?php echo $row["Timing"]; ?> " placeholder="timing" />
													</div>
													<!--end: Code-->
												</div>
