...
<head>
		...
		<meta charset="utf-8" />
		<title>Manage Bus | Update Bus</title>
								...
								<div class="d-flex align-items-center flex-wrap mr-1">
									...
									<div class="d-flex align-items-baseline flex-wrap mr-5">
										...
										<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
											...
											<li class="breadcrumb-item">
												<a href="#" class="text-muted">Update Bus</a>
											</li>
									...
									<div class="col-6">
										...
										<div class="card card-custom example example-compact">
											<div class="card-header">
												<h3 class="card-title">Update Bus</h3>
											...
											if(isset($_POST["btnsubmit"]))
											{
												...
												$ext = pathinfo($_FILES["busimg"]["name"],PATHINFO_EXTENSION);
												if($ext==""){
													$filename=$_POST["busoldimg"];
												}
												else{
													$filename = time().rand(1111,9999).".".$ext; // 1138991234.png

												// echo $filename;
												move_uploaded_file($_FILES["busimg"]["tmp_name"], "uploads/bus/".$filename);
												}
												

												$busno = $_POST["txtbusnumber"];
												$type = $_POST["txtbustype"];
												$isactive = $_POST["txtisactive"];
												$bus_photo= $filename;


												$result = mysqli_query($conn,"update tbl_bus set Bus_no = '".$busno."', Bus_photos='".$filename."', Bus_type='".$type."', Bus_is_active='".$isactive."' where Bus_id = '".$id."'") or die(mysqli_error($conn));
												if($result==true)
												{
													echo "<script>window.location='all_bus.php'</script>";
												}
												else
												{
													echo "Error!";
											...
											<form class="form" method="post" enctype="multipart/form-data">
												<?php
												$count=1;
												$result = mysqli_query($conn,"select * from tbl_bus where Bus_id='".$id."'") or die(mysqli_error($conn));
												...
												<div class="card-body">
													<div class="form-group">
														...
														<input type="text" name="txtbusnumber" class="form-control" value="<?php echo $row["Bus_no"]; ?>" placeholder="Bus Number" />
													
													</div>
													<div class="form-group">
														<label>Bus Photo:</label>
														<input type="file" class="form-control" placeholder="Bus Photo" name="busimg" />
														
															<img src="uploads/bus/<?php echo $row["Bus_photos"]?>" height="100px" width="150px" />
														<input type="hidden" name="busoldimg" value="<?php echo $row["Bus_photos"]?>"/>
													</div>
												
													<div class="form-group">
														<label>Bus Type:</label>
														<select name="txtbustype" class="form-control">
															<option <?php if($row["Bus_type"]=="Electric"){?>selected="true" <?php } ?>>Electric</option>
															<option <?php if($row["Bus_type"]=="Normal"){?>selected="true" <?php } ?>>Normal</option>
														</select>
													</div>
													<div class="form-group">
														<label>Is Active?:</label>
														<select name="txtisactive" class="form-control">
															<option <?php if($row["Bus_is_active"]=="Yes"){?>selected="true" <?php } ?>>Yes</option>
															<option <?php if($row["Bus_is_active"]=="No"){?>selected="true" <?php } ?>>No</option>
														</select>
													</div>
